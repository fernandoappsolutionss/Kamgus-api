...
class WebhooksController extends Controller
{
    ...
    public function pagoCashStatus(Request $request){
      ...
      if(!empty($request->CodOper)){
        ...
        $transaction->receipt_url = "PCPEND_".floor($transaction->accumulated_value * 100.0 / ($transaction->total + $transaction->tax));
        $transaction->description += $fecha." ".$hora."\n";
        if($transaction->accumulated_value >= ($transaction->total + $transaction->tax)){
