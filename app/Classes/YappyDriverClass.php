...
/**
 * YAPPY_ID_DEL_COMERCIO=ID // ID del comercio
 * YAPPY_CLAVE_SECRETA=CLAVE // Clave secreta
 * YAPPY_MODO_DE_PRUEBAS=false // Al colocar el valor true, se realizarán compras de pruebas
 * YAPPY_PLUGIN_VERSION=P1.0.0 // Versión actual del plugin de Yappy
 */
use App\Classes\Bg\BgFirma;
class YappyDriverClass 
{
    private $protocol;
    ...
    //const SUCCESSURL = "https://app.kamgus.com/api/index.php/kamgus/yappy_success";
    private $SUCCESSURL = '';
    //const FAILURL = "https://app.kamgus.com/api/index.php/kamgus/yappy_fail";
    private $FAILURL = '';
    private static $instance = null;

    const DOMAIN = "https://kamgus.com.pa";
    function validateHash()
    {
        try {
            ...
            $hash = $_GET['hash'];
            $domain = $_GET['domain'];
            $values = base64_decode(env("YAPPY_CLAVE_SECRETA"));
    ...
    {
        // verificar credenciales
        $this->SUCCESSURL = url("api/v2/driver/transaction_status/yappy_success");
        $this->FAILURL = url("api/v2/driver/transaction_status/yappy_fail");
    ...
    public function startPayment(){
        $this->response = BgFirma::checkCredentials(env("YAPPY_ID_DEL_COMERCIO"), env("YAPPY_CLAVE_SECRETA"), $this->domain);
        //die( $this->domain);
        //die(json_encode([env("YAPPY_ID_DEL_COMERCIO"), env("YAPPY_CLAVE_SECRETA"), $this->domain]));
        ...
        if ($this->response && $this->response['success']) {
            ...
            $bg = new BgFirma(
                ...
                $this->domain,
                env("YAPPY_CLAVE_SECRETA"),
                env("YAPPY_MODO_DE_PRUEBAS"),
    ...
    public function setPaymenParameters($total, $subtotal, $taxes, $orderId, $tel){
        ...
        $this->paymentParameters["tel"] = $tel;
        return $this;
    }
    public static function getInstance(){
        if(empty(self::$instance)){
            self::$instance = new YappyDriverClass();
        }
